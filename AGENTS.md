...
| Native module missing at runtime | Needs a dev build, not Expo Go. `expo-sqlite`, `expo-crypto`, (since item #12's home screen trend chart) `react-native-svg`, and (since item #15's transactions list) `@shopify/flash-list` all need a native rebuild after install — a stale dev client fails to resolve the newest one |
