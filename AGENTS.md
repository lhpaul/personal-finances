...
1. Credentials live only in `expo-secure-store`, only in the scraper's memory during a sync.
   Never in SQLite, never in a log, never in an error payload. The same rule covers the
   database's own encryption key (`db_key:main`, item #25): `expo-secure-store` only, never
   logged, never echoed in an error message.
...
| Native module missing at runtime (`Cannot find native module 'ExpoSecureStore'`, or the app runs but a native feature silently no-ops) | Needs a dev build, not Expo Go. `expo-sqlite`, `expo-crypto`, `react-native-svg` (item #12's trend chart), `expo-secure-store` (item #9), `react-native-webview` (item #11's `bank-syncing` screen), `@shopify/flash-list` (item #15's transactions list) and `expo-notifications` (item #18's reminders) all need a native rebuild after install — the *compiled binary* does not contain the newest native module until the dev client is rebuilt |
| `Unable to resolve "react-native-webview"` (or any other newly-added package) at Metro bundle time | Metro's package-resolution graph, not the native binary — a stale Metro cache, or a `node_modules` tree that was not re-hoisted after the install. Run `pnpm check:layout` to confirm the tree is still hoisted, then restart Metro with `pnpm dev:mobile --clear` before assuming a native rebuild is needed |
...
| The syncing screen (`#screen=bank-syncing`) sits on the first step forever | The bank changed a selector. Run `pnpm --filter @finanzas/bank-scraper test` — the fixture tests fail before the app does. Re-capture and scrub a fixture, then fix the script |
