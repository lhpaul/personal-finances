...
| Native module missing at runtime | Needs a dev build, not Expo Go. `expo-sqlite`, `expo-crypto`, `react-native-svg` (item #12's trend chart), `expo-secure-store` (item #9), `react-native-webview` (item #11's `bank-syncing` screen) and `@shopify/flash-list` (item #15's transactions list) all need a native rebuild after install — a stale dev client fails to resolve the newest one, with `Cannot find native module 'ExpoSecureStore'` for the secure-store case and `Unable to resolve "react-native-webview"` for the syncing screen |
