...
| Native module missing at runtime (`Cannot find native module 'ExpoSecureStore'`, or the app runs but a native feature silently no-ops) | Needs a dev build, not Expo Go. `expo-sqlite`, `expo-crypto`, `react-native-svg` (item #12's trend chart), `expo-secure-store` (item #9), `react-native-webview` (item #11's `bank-syncing` screen) and `@shopify/flash-list` (item #15's transactions list) all need a native rebuild after install — the *compiled binary* does not contain the newest native module until the dev client is rebuilt |
