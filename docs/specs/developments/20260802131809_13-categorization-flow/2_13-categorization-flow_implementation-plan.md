...
| `useFocusEffect` is importable from the installed router | `grep -n "useFocusEffect" apps/mobile/node_modules/expo-router/build/exports.d.ts` | `export { useFocusEffect, EffectCallback } from './useFocusEffect';` — present in the installed `expo-router@6.0.24`, so the focus-driven reload of Decision 16 needs no new dependency |
