...
describe('plantCredentialEntry / clearConnectFixtures — credential isolation (found in review)', () => {
  ...
  it('restores a real pre-existing Banco de Chile credential after plant + clear', async () => {
    ...
    const port = createFakePort({ [credentialsKeyFor('banco-de-chile')]: realCredential });

    await plantCredentialEntry(port);
    // The fixture value is in place while the panel is being used.
    await expect(port.getItem(credentialsKeyFor('banco-de-chile'))).resolves.not.toBe(realCredential);

    await clearConnectFixtures(port);
    await expect(port.getItem(credentialsKeyFor('banco-de-chile'))).resolves.toBe(realCredential);
  });
  ...
  it('deletes the fixture credential on clear when there was nothing before it', async () => {
    ...
    await plantCredentialEntry(port);
    await expect(port.getItem(credentialsKeyFor('banco-de-chile'))).resolves.not.toBeNull();
  ...
  it('never touches a real Santander credential that this module did not itself write', async () => {
    ...
    const port = createFakePort({ [credentialsKeyFor('santander')]: realSantanderCredential });
    ...
    await expect(port.getItem(credentialsKeyFor('santander'))).resolves.toBe(realSantanderCredential);
  });
